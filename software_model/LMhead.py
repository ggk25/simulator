...
class LMhead:
    def __init__(self, device: Device, vocab_size: int, hidden_size: int, datatype):
        ...
        self.W_lmhead = Tensor([self.hidden_size, self.vocab_size], self.datatype)
        self.lmhead = matmul(self.datatype)

    def __call__(self, input: Tensor)-> Tensor:
        b, s, d = input.shape
        assert d == self.hidden_size
        output = self.lmhead(input, self.W_lmhead)
        return output
    ...
    def mapping_and_simulate(self, device: Device, layer_id: int = 0, micro_batch_id: int = 0):
        ...
        def _calc_dram_energy(op_name: str, duration_cycles: float) -> float:
            weight_bytes = 0
            if op_name == "LMhead":
                weight_bytes = size(self.W_lmhead.shape) * self.datatype.word_size
        ...
        t, last_out = measure("LMhead", self.lmhead, layer_id=layer_id, micro_batch_id=micro_batch_id)
        total_latency += t
